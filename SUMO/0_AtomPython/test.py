    ...
    with open("data/cross.rou.xml", "w") as routes:
        ...
        for i in range(N):
            if random.uniform(0, 1) < pWE:
                print('    <vehicle id="right_%i" type="typeWE" route="right" \
                depart="%i" />' % (
                    vehNr, i), file=routes)
                vehNr += 1
            if random.uniform(0, 1) < pEW:
                print('    <vehicle id="left_%i" type="typeWE" route="left" \
                depart="%i" />' % (
                    vehNr, i), file=routes)
                vehNr += 1
            if random.uniform(0, 1) < pNS:
                print('    <vehicle id="down_%i" type="typeNS" route="down" \
                depart="%i" color="1,0,0"/>' % (
                    vehNr, i), file=routes)
                vehNr += 1
...
def run():
    ...
    traci.trafficlight.setPhase("cluster_1363831222_208533277_528497077", 2)
    while traci.simulation.getMinExpectedNumber() > 0:
        traci.simulationStep()
        if traci.trafficlight.getPhase(
                    "cluster_1363831222_208533277_528497077") == 2:
            # we are not already switching
            if traci.inductionloop.getLastStepVehicleNumber("myDetector") > 0:
                # there is a vehicle from the north, switch
                traci.trafficlight.setPhase(
                        "cluster_1363831222_208533277_528497077", 3)
            else:
...
if __name__ == "__main__":
    ...
    traci.start([sumoBinary, "-c", "data/toronto.sumocfg",
